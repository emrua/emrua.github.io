...
$(function(){
    ...
    $("#header").load("components/header.html", function() {
        // Get the page title and update the header
        var pageTitle = document.title.split("||")[2].trim(); // Extract page title from the full document title
        $("#pageName").text(pageTitle);
    });
});

document.getElementById("validation_link_disability").setAttribute("href","https://wave.webaim.org/report#/" + location.href);

function toggleCSS() {
    var styleSheets = document.querySelectorAll('link[rel=stylesheet]');
    styleSheets.forEach(function(sheet) {
        if (sheet.getAttribute('href') !== '') {
            sheet.setAttribute('data-href', sheet.getAttribute('href'));
            sheet.setAttribute('href', '');
        } else {
            sheet.setAttribute('href', sheet.getAttribute('data-href'));
        }
    });
}